...
export class LinkMaintenanceSaleUseCase {
  ...
  execute(id: string, payload: { saleId?: string; folio?: string }): Promise<MaintenanceOrder> {
    if (!id || !id.trim()) {
      throw new Error('El ID de la orden de mantenimiento es requerido');
    }
    return this.maintenanceRepository.linkMaintenanceSale(id.trim(), payload);
  }
}
