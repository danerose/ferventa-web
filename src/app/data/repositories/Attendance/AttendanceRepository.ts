...
export class APIAttendanceRepository {
   ...
   */
  async getBranchTodayStatus(branchId?: string, date?: string): Promise<BranchTodayStatus> {
    ...
    return (json.data?.data ?? json.data) as BranchTodayStatus;
  }
  ...
  async getMyRecords(startDate?: string, endDate?: string): Promise<AttendanceRecord[]> {
    ...
    return (json.data || []).map((r: AttendanceRecord & { _id?: string }) => ({
      ...r,
      id: r.id || r._id || '',
    }));
  }
