...
export class APIUserRepository {
  ...
  async getRoles(token: string): Promise<Role[]> {
    ...
    return (json.data || []).map((r: Role & { _id?: string }) => ({
      ...r,
      id: r.id || r._id || '',
    }));
  }
  ...
  async getUsers(token: string): Promise<User[]> {
    ...
    return (json.data || []).map((u: User & { _id?: string; role?: { id?: string; _id?: string; name: string } }) => ({
      ...u,
      id: u.id || u._id || '',
  ...
  async createUser(token: string, data: CreateUserDto): Promise<CreateUserResponse> {
    ...
    if (!payload.email || !String(payload.email).trim()) delete payload.email;
