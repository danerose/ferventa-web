...
export const AttendancePage: React.FC = () => {
  ...
  // Admin summary state
  const [period, setPeriod] = useState<'weekly' | 'biweekly' | 'monthly' | 'custom'>('weekly');
  ...
  // Fetch all users & filter for active branch
  useEffect(() => {
    const loadUsersAndBranch = async () => {
      setIsUsersLoading(true);
      try {
        if (!accessToken) return;
        const allUsers = await userRepo.getUsers(accessToken);
        const allBranches = await adminRepo.getBranches();
        setBranches(allBranches || []);

        // Filter users belonging to activeBranchId (or fallback if non-restricted)
        if (activeBranchId && activeBranchId !== '000000000000000000000000') {
          ...
          setBranchUsers(filtered.length > 0 ? filtered : allUsers);
        } else {
          setBranchUsers(allUsers || []);
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  // Load Admin Data (Summary & Global Records)
  ...
  }, [isAdmin, activeTab, selectedBranchId, period, statusFilter]);
  ...
  const handleOpenBreakdown = (userSum: AttendanceUserSummary) => {
    setSelectedUserForBreakdown({ id: userSum.userId, name: userSum.userName });
  ...
  // Filter out ADMIN users from the checador cards list (non-admins only)
  ...
  const safeUserSummary = Array.isArray(summaryData?.usersSummary) ? summaryData!.usersSummary : [];
    ...
    <div style={{ background: '#f8f9ff', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ marginLeft: '240px', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '28px', maxWidth: '1280px', width: '100%', margin: '0 auto' }}>
          ...
          {/* Navigation Tabs (Admin only gets tab bar to switch views, regular user stays on Mi Checador) */}
          {isAdmin && (
          ...
          {/* TAB 1: Checador de Sucursal por Colaborador */}
          {(activeTab === 'my-clock' || !isAdmin) && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              
              {/* Branch Clock & Date Header Card */}
              ...
              {isUsersLoading ? (
                ...
                <div style={{ padding: '40px', textAlign: 'center', background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', color: '#64748b', fontSize: '14px' }}>
                  No hay colaboradores registrados para marcar en esta sucursal.
                </div>
              ) : (
          ...
          {isAdmin && activeTab === 'admin-dashboard' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              {/* Filter Bar */}
              <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      style={{ padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '14px', outline: 'none', background: 'white', cursor: 'pointer', minWidth: '180px' }}
                    >
                  ...
                  {/* Period selector */}
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', marginBottom: '4px' }}>Periodo de Resumen</label>
                    <select
                      value={period}
                      onChange={(e) => setPeriod(e.target.value as any)}
                      style={{ padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '14px', outline: 'none', background: 'white', cursor: 'pointer', minWidth: '160px' }}
                    >
                ...
                <SecondaryButton onClick={loadAdminDashboardData} size="sm">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
                <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                  ...
                  <span style={{ fontSize: '24px', fontWeight: '800', color: '#16a34a', fontFamily: 'monospace' }}>
                    {formatMinutes(
                      safeUserSummary.reduce((acc, u) => acc + (u.netWorkMinutes || 0), 0)
                    )}
                  </span>
                </div>
                ...
                <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                  ...
                  <span style={{ fontSize: '24px', fontWeight: '800', color: '#d97706', fontFamily: 'monospace' }}>
                    {formatMinutes(
                      safeUserSummary.reduce((acc, u) => acc + (u.totalBreakMinutes || 0), 0)
                    )}
                  </span>
                </div>
                ...
                <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                  ...
                  <span style={{ fontSize: '24px', fontWeight: '800', color: '#091426', fontFamily: 'monospace' }}>
                    {safeUserSummary.reduce((acc, u) => acc + (u.completedShifts || 0), 0)}
                  </span>
                </div>

                <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                  <span style={{ fontSize: '12px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Colaboradores Activos</span>
                  <span style={{ fontSize: '24px', fontWeight: '800', color: '#6b21a8', fontFamily: 'monospace' }}>
                    {safeUserSummary.length}
                  </span>
                </div>
              </div>
              ...
              <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0' }}>
                  <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#0f172a', margin: 0 }}>
                    Resumen de Horas Trabajadas por Usuario
                  </h3>
                  {summaryData?.range && (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                      <tr>
                        ...
                        <th style={{ padding: '12px 16px', textAlign: 'center', fontSize: '12px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Turnos Completados</th>
                        <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Horas Brutas</th>
                        ...
                        <th style={{ padding: '12px 16px', textAlign: 'right', fontSize: '12px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Acciones</th>
                      </tr>
                    </thead>
                    <tbody>
                      {safeUserSummary.map((uSum) => (
                        <tr key={uSum.userId} style={{ borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          <td style={{ padding: '14px 16px', textAlign: 'center', fontSize: '14px', fontWeight: '700', color: '#0f172a', fontFamily: 'monospace' }}>
                            {uSum.completedShifts} / {uSum.totalShifts}
                          </td>
                          <td style={{ padding: '14px 16px', fontSize: '14px', color: '#475569', fontFamily: 'monospace' }}>
                            {formatMinutes(uSum.totalWorkMinutes)}
                          </td>
                          ...
                          <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                            <SecondaryButton size="sm" onClick={() => handleOpenBreakdown(uSum)}>
                              <Icon name="Eye" size="xs" className="mr-1" /> Ver Desglose
                            </SecondaryButton>
                          </td>
                        </tr>
                      ))}
                          ...
                          <tr key={rec._id || rec.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                            ...
                            <td style={{ padding: '14px 16px' }}>
                              <span style={{
                                ...
                                padding: '2px 8px',
                                borderRadius: '4px',
                                background: rec.status === 'completed' ? '#dcfce7' : rec.status === 'on_break' ? '#fef3c7' : '#dbeafe',
                                color: rec.status === 'completed' ? '#166534' : rec.status === 'on_break' ? '#92400e' : '#1e40af'
                              }}>
                                {rec.status === 'completed' ? 'Completado' : rec.status === 'on_break' ? 'En Descanso' : 'En Turno'}
                              </span>
                            </td>
                            <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                              <button
